...
inventors.map((inventor) => {
    let inventorName = {first: inventor.first , last: inventor.last}
    inventorNames.push(inventorName)
});
...
isAdultPresent = devs.some((dev) => (2024-dev.year) >= 18);
...
isEveryone19OrOlder = devs.every((dev) => (2024-dev.year) > 18);
...
travelMethodCounts = travelMethods.reduce(function(acc, meth){
    if (acc[meth]){
        acc[meth] = acc[meth] +1;
    } else {
        acc[meth] = 1
